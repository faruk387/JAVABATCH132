...
public class LambdaForTextFiles {
    public static void main(String[] args) throws IOException {
      ...
      boolean r1=Files.lines(Paths.get("src/main/java/day33lambda/MyTextFile.txt")).anyMatch(t->t.contains("Java"));
        ...
        Files.lines(Paths.get("src/main/java/day33lambda/MyTextFile.txt")).map(t->t.split(" ")).flatMap(Arrays::stream).distinct().forEach(System.out::println);
        //Get the words which end with "s" and print them on the console
        Files.lines(Paths.get("src/main/java/day33Lambda/MyTextFile.txt")).map(t->t.split(" ")).flatMap(Arrays::stream).filter(t->t.endsWith("s")).forEach(System.out::println);

        System.out.println("==========");
        //Get the words distinctly, convert them to lowercase and print them on the console in reverse order
        Files.lines(Paths.get("src/main/java/day33lambda/MyTextFile.txt")).map(t->t.split(" ")).flatMap(Arrays::stream).map(t->t.replaceAll("[^a-zA-Z]","")).map(t->t.toLowerCase()).distinct().sorted(Comparator.reverseOrder()).forEach(System.out::println);
        System.out.println("==========");
